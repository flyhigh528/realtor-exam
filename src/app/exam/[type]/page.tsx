    ...
    <>
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-4 py-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
          <h1 className="text-xl font-bold text-gray-900 mb-1">{config.label}</h1>
          <p className="text-sm text-gray-500 mb-6">{config.description}</p>

          {/* 회차 선택 */}
          {maxRounds > 1 && (
            <div className="mb-6">
              <label className="text-sm font-semibold text-gray-800 block mb-2">회차 선택</label>
          ...
          <div className="space-y-3 mb-6">
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">시험 과목</span>
              <span className="font-medium text-gray-900">{subjects.length}과목</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">총 문항 수</span>
              <span className="font-medium text-gray-900">{config.totalQuestions}문항</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">시험 시간</span>
              <span className="font-medium text-gray-900">{minutes}분</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">문항당 배점</span>
              <span className="font-medium text-gray-900">2.5점</span>
            </div>
          </div>

          <div className="border-t border-gray-100 pt-4">
            <h3 className="text-sm font-semibold text-gray-800 mb-2">시험 과목</h3>
            <div className="space-y-2">
              {subjects.map((subject) => (
                <div key={subject.id} className="flex justify-between text-sm bg-gray-50 rounded-lg px-3 py-2">
                  <span className="text-gray-700">{subject.name}</span>
                  <span className="text-gray-500">{subject.questionCount}문항</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-amber-50 rounded-xl p-4 mb-6 border border-amber-100">
          <h3 className="text-sm font-semibold text-amber-800 mb-2">유의사항</h3>
